...
## Installing Youtrack
  ...
  1. Add Helm Repo
      ...
      ```bash
      helm repo add getyoutrack https://helm.youtrack.so/
      ```

  1. Set-up and customization

      - Quick set-up<br>
        This is the fastest way to deploy Youtrack with default settings. This will create stateful deployments for Postgres, Redis, and Minio with a persistent volume claim using the `longhorn` storage class. This also sets up the ingress routes for you using `nginx` ingress class.
        ...
        ```bash
          helm upgrade --install youtrack-app getyoutrack/youtrack-ce \
              --create-namespace \
              --namespace youtrack-ce \
              --set youtrackVersion=stable \
              --set ingress.appHost="youtrack.example.com" \
              --set ingress.minioHost="youtrack-minio.example.com" \
              --set ingress.rabbitmqHost="youtrack-mq.example.com" \
        ...
        > This is the minimum required to set up Youtrack-CE. You can change the default namespace from `youtrack-ce`, the default appname
        from `youtrack-app`, the default storage class from `[postgres, redis, minio].storageClass`, and the default ingress class from `ingress.ingressClass` to whatever you would like to.<br> <br>
      ...
      - Advance set-up<br>
        ...
        ```bash
        helm  show values getyoutrack/youtrack-ce > values.yaml
        vi values.yaml
        ```
...
| env.pgdb_remote_url |  |  | Users can also decide to use the remote hosted database and link to Youtrack deployment. Ignoring all the above keys, set `postgres.local_setup` to `false` and set this key with remote connection url. |
...
| env.remote_redis_url |  |  | Users can also decide to use the remote hosted database and link to Youtrack deployment. Ignoring all the above keys, set `redis.local_setup` to `false` and set this key with remote connection url. |
...
| rabbitmq.default_user | youtrack |  | Credentials are requried to access the hosted stateful deployment of `rabbitmq`.  Use this key to set the username for the stateful deployment. |
| rabbitmq.default_password | youtrack |  | Credentials are requried to access the hosted stateful deployment of `rabbitmq`.  Use this key to set the password for the stateful deployment. |
| rabbitmq.assign_cluster_ip | false |  | Set it to `true` if you want to assign `ClusterIP` to the service |
| rabbitmq.external_rabbitmq_url |  |  | Users can also decide to use the remote hosted service and link to Youtrack deployment. Ignoring all the above keys, set `rabbitmq.local_setup` to `false` and set this key with remote connection url. |

### Doc Store (Minio/S3) Setup

| Setting | Default | Required | Description |
|---|:---:|:---:|---|
| minio.local_setup | true |  | Youtrack uses `minio` as the default file storage drive. This storage can be hosted within kubernetes as part of helm chart deployment or can be used as hosted service remotely (e.g. aws S3 or similar services). Set this to  `true` when you choose to setup stateful deployment of `postgres`. Mark it as `false` when using a remotely hosted database |
...
| web.image| getyoutrack/youtrack-frontend |  |  This deployment needs a preconfigured docker image to function. Docker image name is provided by the owner and must not be changed for this deployment |
...
| space.image| getyoutrack/youtrack-space|  |  This deployment needs a preconfigured docker image to function. Docker image name is provided by the owner and must not be changed for this deployment |
...
| admin.image| getyoutrack/youtrack-admin |  |  This deployment needs a preconfigured docker image to function. Docker image name is provided by the owner and must not be changed for this deployment |
...
| live.image| getyoutrack/youtrack-live |  |  This deployment needs a preconfigured docker image to function. Docker image name is provided by the owner and must not be changed for this deployment |
...
| api.image| getyoutrack/youtrack-backend |  | This deployment needs a preconfigured docker image to function. Docker image name is provided by the owner and must not be changed for this deployment |
...
| worker.image| getyoutrack/youtrack-backend |  | This deployment needs a preconfigured docker image to function. Docker image name is provided by the owner and must not be changed for this deployment |
...
| beatworker.image| getyoutrack/youtrack-backend |  | This deployment needs a preconfigured docker image to function. Docker image name is provided by the owner and must not be changed for this deployment |
...
| ingress.appHost | 'youtrack.example.com' | Yes | The fully-qualified domain name (FQDN) in the format `sudomain.domain.tld` or `domain.tld` that the license is bound to. It is also attached to your `ingress` host to access Youtrack. |
| ingress.minioHost | 'youtrack-minio.example.com' |  | Based on above configuration, if you want to expose the `minio` web console to set of users, use this key to set the `host` mapping or leave it as `EMPTY` to not expose interface. |
| ingress.rabbitmqHost | 'youtrack-mq.example.com' |  | Based on above configuration, if you want to expose the `rabbitmq` web console to set of users, use this key to set the `host` mapping or leave it as `EMPTY` to not expose interface. |
